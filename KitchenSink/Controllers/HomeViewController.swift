...
class HomeViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    fileprivate enum Feedback: CaseIterable {
        static let recipient = "devsupport@ciscospark.com"
...
//                self.present(alert, animated: true)
    ...
    private lazy var ucConnectionStatusLabel: UILabel = {
        ...
        label.text = "UC Server Connected"
        label.translatesAutoresizingMaskIntoConstraints = false
        label.textColor = .momentumGreen50
    ...
    func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            ucConnectionStatusLabel.topAnchor.constraint(equalTo: view.topAnchor, constant: 150)
        ])
    }
}
...
extension HomeViewController {
    ...
    public func ucServerConnectionSuccess() {
...
extension HomeViewController {
    ...
    private func sendMailViaComposer(_ feedback: Feedback) {
        ...
        // add log files as attachment
        let logFileUrls = webex.getLogFileUrls()
        
        for fileUrlKey in logFileUrls.keys {
            if  let fileUrl = logFileUrls[fileUrlKey], let fileContents = NSData(contentsOf: fileUrl) {
                controller.addAttachmentData(fileContents as Data, mimeType: "text/plain", fileName: "\(fileUrlKey).log")
            }
        }
...
extension HomeViewController {
    func deviceRegisteration() {
        webex.people.getMe(completionHandler: { result in
            guard let person = result.data, let emailId = person.emails?[0].toString(), let personId = person.id else { return }
