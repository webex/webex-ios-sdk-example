...
class SearchContactViewController: UIViewController, UITableViewDataSource, UISearchBarDelegate {
    ...
    private func fetchResults(for text: String) {
        webex.getSearchResult(searchString: text, completionHandler: { [weak self] searchResult in
            let result = searchResult.map { Space(id: $0.conversationId, title: $0.title) }
