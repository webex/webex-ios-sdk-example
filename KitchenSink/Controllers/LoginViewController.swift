...
class LoginViewController: UIViewController {
    ...
    private lazy var loginWithOAuthCode: UIButton = {
        let view = UIButton(type: .system)
        view.translatesAutoresizingMaskIntoConstraints = false
        view.addTarget(self, action: #selector(handleLoginWithOAuthCode), for: .touchUpInside)
        view.setTitle("Login with Auth code", for: .normal)
        view.titleLabel?.font = .preferredFont(forTextStyle: .title3)
        view.setTitleColor(.white, for: .normal)
        view.backgroundColor = .momentumBlue50
        view.setHeight(50)
    ...
    func initializeWebex() {
        webex.logger = KitchenSinkLogger()
    ...
    func initWebexUsingOauth() {
        ...
        webex = Webex(authenticator: OAuthAuthenticator(clientId: clientId, clientSecret: clientSecret, redirectUri: redirectUri, emailId: ""))
        initializeWebex()
    }
    ...
    @objc private func handleLoginWithJWTAction() {
        ...
        let alert = UIAlertController(title: "Guset Login", message: "Enter JWT token", preferredStyle: .alert)
    ...
    @objc private func handleLoginWithOAuthCode() {
        initWebexUsingOauth()
        loginWithOAuthCode.alpha = 0.7
        loginWithOAuthCode.setTitle("Loading...", for: .normal)
        loginWithOAuthCode.isEnabled = false
        
        let alert = UIAlertController(title: "Login", message: "Enter Auth code", preferredStyle: .alert)
        
        alert.addTextField { textField in
            textField.placeholder = "Enter Auth code"
            textField.text = ""
        }
        if let authenticator = webex.authenticator as? OAuthAuthenticator {
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { [weak alert] _ in
                let textField = alert?.textFields![0] // Force unwrapping because we know it exists.
                authenticator.authorize(oauthCode: textField?.text ?? "", completionHandler: { success in
                    guard success else {
                        self.loginWithOAuthCode.isEnabled = true
                        print("Login failed")
                        return
                    }
                    UserDefaults.standard.setValue("auth", forKey: "loginType")
    ...
    func setupViews() {
        ...
        view.addSubview(loginWithOAuthCode)
    ...
    func setupConstraints() {
        ...
        loginWithJWTButton.topAnchor.constraint(equalTo: loginWithOAuthCode.bottomAnchor, constant: 20).activate()
        ...
        loginWithOAuthCode.topAnchor.constraint(equalTo: loginButton.bottomAnchor, constant: 20).activate()
        loginWithOAuthCode.centerXAnchor.constraint(equalTo: view.centerXAnchor).activate()
        loginWithOAuthCode.fillWidth(of: view, padded: 64)
