...
extension FilterSpacesViewController {
    ...
    @objc private func onDoneTapped() {
        ...
        if let teamIdText = teamId.text, !teamIdText.isEmpty {
            filterTeamId = teamId.text
        }
        delegate?.didUpdateListParameters(teamId: filterTeamId, maxNumberOfSpaces: Int(maxNumberOfSpaces.text ?? "0"), spaceType: selectedSpaceType, sortType: selectedSpaceSortType, spaceValueType: segementedControl.selectedSegmentIndex == 0 ? .space : .readStatus)
