...
class ScheduledMeetingViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getUpdatedSchedule(call: Call) {
        guard let callSchedule = call.schedules else {
            ...
            let newCall = Meeting(organizer: call.title ?? "", start: Date(), end: Date(), meetingId: "", link: "", subject: "", isScheduledCall: false, space: Space(id: call.conversationId ?? "", title: call.title ?? ""), currentCallId: currentCallId ?? "")
        ...
        for item in callSchedule {
            let newMeetingId = Meeting(organizer: item.organzier ?? "", start: item.start, end: item.end, meetingId: item.meetingId ?? "", link: item.link ?? "", subject: item.subject ?? "", isScheduledCall: true, space: Space(id: call.conversationId ?? "", title: call.title ?? ""), currentCallId: call.callId ?? "")
            ...
            if !isExistingScheduleModified {
                ...
                self.tableData.append(Meeting(organizer: item.organzier ?? "", start: item.start, end: item.end, meetingId: item.meetingId ?? "", link: item.link ?? "", subject: item.subject ?? "", isScheduledCall: true, space: Space(id: call.conversationId ?? "", title: call.title ?? ""), currentCallId: call.callId ?? ""))
                break
            }
        }
    }
